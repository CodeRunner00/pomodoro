...
class App extends Component {
  ...
  handlePassiveChange(e) {
    this.setState({
      typingActiveTime: "true"
    });

    this.setState({
      passiveTime: parseInt(e.target.value)
    });
  }
  ...
  handleActiveChange(e) {
    ...
    this.setState({
      activeTime: parseInt(e.target.value)
    });
    this.setState({
      time: this.miliseconds(0, parseInt(e.target.value), 0)
    });
  }
  ...
  resetActiveTime = () => {
    this.setState({
      time: this.miliseconds(0, this.state.activeTime, 0)
    });
    this.setState({
      playText: "Time has started!"
    });
  };

  resetPassiveTime = () => {
    this.setState({
      time: this.miliseconds(0, this.state.passiveTime, 0)
    });
    this.setState({
      playText: "Relax!"
    });
  ...
  runActiveTime = () => {
    ...
    this.setState({
      activeInterval: setInterval(this.countdownFunction, 1000)
    });
    this.setState({
      playText: "Go GO GO"
    });
  ...
  render() {
    return (
      <div className="App">
        ...
        <ActivePomodoroInput
          handleActiveSubmit={this.handleSubmit}
          handleActiveChange={this.handleActiveChange}
          active={this.state.active}
        />{" "}
        <br />
        <PassivePomodoroInput
          handleActiveSubmit={this.handleSubmit}
          handlePassiveChange={this.handlePassiveChange}
          active={this.state.active}
        />{" "}
        <br />
        <p
          className={`show-typing-${this.state.typingActiveTime} ${
            this.state.pomodoroStarted
          }`}
        >
          {" "}
          Press get started button when you 've entered active and passive times
        </p>{" "}
        <Button
          onClick={this.runActiveTime}
          playText={this.state.playText}
          started={this.state.active}
        />{" "}
        <hr />
